    ...
    <div className="min-h-full bg-gradient-to-b from-[#2e1a4a] to-surface-1 px-6 py-10">
      <header className="mb-8">
        ...
        <p className="mt-2 text-text-secondary">
          Your episodes and shows will appear here.
        </p>
      </header>

      {["Your Shows", "New Episodes", "Popular Podcasts"].map((shelf) => (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="mb-3 aspect-square w-full rounded-full bg-surface-3 transition-colors group-hover:bg-surface-4" />
